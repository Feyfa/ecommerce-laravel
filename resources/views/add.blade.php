...
@section('main2')
    <form class="w-full px-4 flex flex-col justify-center gap-5" action="" method="post" enctype="multipart/form-data">
        ...
        <div class="row w-full flex gap-5 flex-col items-center justify-center">
            <img src="{{ asset('imgs/product.png') }}" alt="product" class="product w-[400px] h-[225px] border border-neutral-500 rounded shadow-md cursor-zoom-in">
            ...
            <div class="product-container fixed top-0 left-0 bottom-0 right-0 bg-[rgba(0,0,0,.5)] backdrop-blur-sm hidden justify-center items-center cursor-zoom-out">
                <img src="{{ asset('imgs/product.png') }}" alt="user" class="product w-[560px] h-[315px] border border-neutral-500 rounded shadow-md">
            </div>
        </div>

        <div class="row flex justify-center items-center">
            <input required type="hidden" name="user_id_seller" value="{{ $id }}">
            <input required type="file" name="img" id="img" class="bg-white border border-neutral-500 rounded-sm p-1 w-[19rem] shadow-md img-input">
        </div>

        <div class="grid grid-cols-3 gap-5 text-xl">
            <div class="flex flex-col items-start gap-1">
                <label for="name" class="@error('name') text-red-500 @enderror">Name</label>
                <input required type="text" name="name" id="name" class="border border-neutral-500 rounded-sm py-2 px-2.5 w-full outline-none shadow-md @error('name') border-red-500 @enderror">
                @error('name')
            ...
            <div class="flex flex-col items-start gap-1">
                ...
                <input required type="number" min="1" name="price" id="price" class="border border-neutral-500 rounded-sm py-2 px-2.5 w-full outline-none shadow-md @error('price') border-red-500 @enderror">
                @error('price')
                    <p class="-mt-1 text-base text-red-500">{{ $message }}</p>
                @enderror
            </div>
            <div class="flex flex-col items-start gap-1">
                ...
                <input required type="number" min="1" name="stock" id="stock" class="border border-neutral-500 rounded-sm py-2 px-2.5 w-full outline-none shadow-md @error('stock') border-red-500 @enderror">
                @error('stock')
                    <p class="-mt-1 text-base text-red-500">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="grid grid-cols-3 gap-5">
            <button type="submit" name="tambah_submit" class="py-2.5 border border-neutral-500 rounded-sm shadow-lg bg-blue-500 mt-2 hover:bg-[#428bff]">Tambah</button>
        </div>
    </form>
